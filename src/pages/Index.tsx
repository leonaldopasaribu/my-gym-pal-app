...
import { Dumbbell, ListChecks, LineChart, Trophy, Home } from "lucide-react";
        ...
        {tab === "home" && (
          <section className="mb-5 sm:mb-8 animate-fade-up">
            ...
            <h1 className="font-display text-3xl sm:text-5xl font-bold leading-[1.05] max-w-2xl">
              Lift heavier <span className="text-gradient-primary">every week.</span>
            </h1>
            <p className="text-sm sm:text-base text-muted-foreground mt-2 sm:mt-3 max-w-xl">
              Custom exercises, log sets/reps/weight, and visualize progressive overload.
            </p>
          </section>
        )}

        <Tabs value={tab} onValueChange={setTab} className="space-y-5 sm:space-y-6">
          ...
          <TabsList className="hidden md:flex bg-secondary/40 border border-border/60 p-1 h-auto flex-wrap">
            ...
            <TabsTrigger value="log" className="gap-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
            ...
            <TabsTrigger value="progress" className="gap-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
              <LineChart className="h-4 w-4" /> Progress
            </TabsTrigger>
            <TabsTrigger value="prs" className="gap-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
              <Trophy className="h-4 w-4" /> PRs
            </TabsTrigger>
          </TabsList>

          <TabsContent value="home" className="mt-0"><HomeView /></TabsContent>
          <TabsContent value="log" className="mt-0"><WorkoutLogger /></TabsContent>
          <TabsContent value="library" className="mt-0"><ExerciseManager /></TabsContent>
          <TabsContent value="progress" className="mt-0"><ProgressView /></TabsContent>
          <TabsContent value="prs" className="mt-0"><PRDashboard /></TabsContent>
          ...
          >
            <MobileTab value="home" icon={<Home className="h-5 w-5" />} label="Home" />
            <MobileTab value="log" icon={<ListChecks className="h-5 w-5" />} label="Log" />
            <MobileTab value="library" icon={<Dumbbell className="h-5 w-5" />} label="Library" />
            <MobileTab value="progress" icon={<LineChart className="h-5 w-5" />} label="Progress" />
            <MobileTab value="prs" icon={<Trophy className="h-5 w-5" />} label="PRs" />
          </TabsList>
        </Tabs>
...
function MobileTab({ value, icon, label }: { value: string; icon: React.ReactNode; label: string }) {
  return (
    <TabsTrigger
      value={value}
